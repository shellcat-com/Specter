...
Twelve original pixel characters for review before native integration. Each uses a 24 × 24 pixel canvas, three colors plus transparency, and twelve frames in each of three states: idle, working, and celebrate. Playback is eight frames per second. These are alternatives to the four existing vector companions; the shipping app and its preferences have not changed.
...
The next milestone can map the chosen design into the existing companion strip, retaining its pause, Off, per-profile persistence, inactive-window behavior, and macOS Reduce Motion support. Keep decorative animation outside the terminal grid and PTY data path. Do not infer command completion by scanning terminal contents. Any future state signal needs an explicit, separately designed source. These twelve options have not yet been wired into `MascotStyle`, settings, or the native app.
