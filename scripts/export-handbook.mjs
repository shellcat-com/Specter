...
async function exportFile(path, content) {
  const url = new URL(path, import.meta.url);
  if (check) {
    const existing = await readFile(url, 'utf8').catch(() => null);
    if (existing !== content) {
      throw new Error(`${path} is stale. Run node scripts/export-handbook.mjs and commit both generated handbooks.`);
    }
  } else {
...
await exportFile('../docs/handbook.md', output);
console.log(`${check ? 'Checked' : 'Exported'} ${guides.length} chapters in docs/handbook.md`);

const styles = `:root{font:15px/1.75 -apple-system,BlinkMacSystemFont,sans-serif;color:#26262a;background:#f8f7f5}body{margin:0}.skip-link{position:absolute;left:20px;top:-60px}.skip-link:focus{top:12px}a{color:#4d51a8}a:focus-visible{outline:3px solid #6468ba;outline-offset:4px}.layout{display:grid;grid-template-columns:220px minmax(0,780px);gap:55px;max-width:1100px;margin:45px auto;padding:0 28px}nav{position:sticky;top:28px;align-self:start;display:flex;flex-direction:column;gap:10px}nav strong{font-size:24px;margin-bottom:15px}nav a{text-decoration:none;font-size:13px}h1{font-size:42px;font-weight:400;letter-spacing:-2px}h2{font-size:30px;font-weight:400;line-height:1.3}h3{font-size:20px;font-weight:500;margin-top:28px}p,li{color:#595962}article{padding:22px 0 38px;border-bottom:1px solid #dedde1}pre{padding:20px;background:#27272f;color:#e8e7ef;border-radius:6px;white-space:pre-wrap}code,kbd{font:12px/1.7 ui-monospace,Menlo,monospace}table{border-collapse:collapse;width:100%;font-size:13px}td,th{text-align:left;padding:12px 8px;border-bottom:1px solid #dedde1;vertical-align:top}.doc-note{padding:16px;border-left:3px solid #7477b9;background:#eeedf6}section{scroll-margin-top:20px}@media(max-width:720px){.layout{grid-template-columns:1fr;gap:25px;margin:24px auto;padding:0 20px}nav{position:static;display:grid;grid-template-columns:1fr 1fr}nav strong{grid-column:1/-1}h1{font-size:34px}}`;
...
const standalone = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Specter — Offline handbook</title><style>${styles}</style></head><body><a class="skip-link" href="#main">Skip to handbook</a><div class="layout"><nav aria-label="Handbook chapters"><strong>specter / handbook</strong>${guides.map(g => `<a href="#${g.id}">${g.title}</a>`).join('')}</nav><main id="main"><h1>Specter handbook</h1><p>This handbook is included with Specter and works offline. No scripts, accounts, or network connection are needed to read it.</p>${standaloneBody}</main></div></body></html>`;
await exportFile('../Resources/Handbook.html', standalone.replace(/[ \t]+$/gm, ''));
console.log(`${check ? 'Checked' : 'Exported'} self-contained Resources/Handbook.html for the app Help menu.`);
